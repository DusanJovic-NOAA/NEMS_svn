...
!
     ...
     &                     kt2(im),  kbcon1(im), ktcon(im), lmin(im),
...
c
     ...
     &                    .5 * (uo(i,k) + uo(i,k-1)) +
     &                    .5 * pgcon * onemfp * (uo(i,k) - uo(i,k-1))
              vcko(i,k) = factor * vcko(i,k-1) + onemf *
     &                    .5 * (vo(i,k) + vo(i,k-1)) +
     &                    .5 * pgcon * onemfp * (vo(i,k) - vo(i,k-1))
...
c
      ...
      do k = 2, km
      do i = 1, im
        if (flg(i).and.k .le. kmax(i)-1) then
...
c
      ...
      do k = 2, km1
        do i = 1, im
          if (cnvflg(i)) then
            if(k.ge.kbcon1(i).and.k.le.kbmax(i)) then
              if(heo(i,k).lt.hmin(i)) then
                 lmin(i) = k + 1
                 hmin(i) = heo(i,k)
              endif
            endif
...
c     endif
      ...
      enddo
c
c  cloud property above cloud base is modified by the entrainment process
c
      do k = 2, km1
        do i = 1, im
          if (k .le. kmax(i)-1) then
     ...
     &                          + (vo(i,k)-vo(i,k-1)) ** 2)
      ...
      do i = 1, im
        ...
        if(cnvflg(i)) then
          ...
          knumb = max(knumb,1)
          dz = .5 * (zo(i,ktcon(i)+1) + zo(i,ktcon(i)) 
     &               - zo(i,kb(i)+1)- zo(i,kb(i)))
          vshear(i) = 1.e3 * vshear(i) / dz
...
cj
      do k = km1, 2, -1
        do i = 1, im
          if (k .le. kmax(i)-1) then
            if(cnvflg(i).and.k.le.jmin(i)) then
              ucdo(i,k-1) = ucdo(i,k) - pgcon * (uo(i,k) - uo(i,k-1))
              vcdo(i,k-1) = vcdo(i,k) - pgcon * (vo(i,k) - vo(i,k-1))
            endif
          endif
        enddo
      enddo
...
c
              ...
              tem1  = 0.5*(eta(i,k)+eta(i,k-1))
              tem2  = 0.5*(etad(i,k)+etad(i,k-1))
cj
...
cj
     ...
     &         - aup*deta*dv2u
     &         + adw*edto(i)*detad*.5*(ucdo(i,k)+ucdo(i,k-1))
     &         - pgcon*(aup*tem1-adw*edto(i)*tem2)*(dv1u-dv3u)
     &         ) *g/dp
cj
     ...
     &         - aup*deta*dv2v
     &         + adw*edto(i)*detad*.5*(vcdo(i,k)+vcdo(i,k-1))
     &         - pgcon*(aup*tem1-adw*edto(i)*tem2)*(dv1v-dv3v)
     &         ) *g/dp
cj
c             tem1=eta(i,k)*(uo(i,k)-ucko(i,k))
c             tem2=eta(i,k-1)*(uo(i,k-1)-ucko(i,k-1))
c             dv1u=etad(i,k)*(uo(i,k)-ucdo(i,k))
c             dv2u=etad(i,k-1)*(uo(i,k-1)-ucdo(i,k-1))
c             dellau(i,k) = dellau(i,k) +
c    &          (aup*(tem1-tem2)-adw*edto(i)*(dv1u-dv2u)) * g / dp
cj
c             tem1=eta(i,k)*(vo(i,k)-vcko(i,k))
c             tem2=eta(i,k-1)*(vo(i,k-1)-vcko(i,k-1))
c             dv1v=etad(i,k)*(vo(i,k)-vcdo(i,k))
c             dv2v=etad(i,k-1)*(vo(i,k-1)-vcdo(i,k-1))
c             dellav(i,k) = dellav(i,k) +
c    &          (aup*(tem1-tem2)-adw*edto(i)*(dv1v-dv2v)) * g / dp
cj
            endif
          endif
        enddo
...
cj
c     do i = 1, im
c     if(me.eq.0.and.lat.eq.latd.and.i.eq.lond
c    &   .and.cnvflg(i)) then
c     if(me.eq.31.and.cnvflg(i)) then
c       print *, '   dellah ='
c       print 6003, (dellah(i,k)*xmb(i),k=1,kmax(i))
c       print *, '   dellaq ='
c       print 6003, (hvap*dellaq(i,k)*xmb(i),k=1,kmax(i))
c       print *, ' deep delhbar, delqbar, deltbar ='
c       print *, delhbar(i),hvap*delqbar(i),cp*deltbar(i)
c       print *, ' deep delubar, delvbar ='
c       print *, delubar(i),delvbar(i)
