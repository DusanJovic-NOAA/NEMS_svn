     ...
     &  dg,tg,zg,ug,vg,rqg,dphi,dlam,ps,zsphi,zslam,
...
c
      ...
      real cons0,cons1,cons2,cons0p5,rdt2
      logical adjusted
      integer levsb
!
! zflxtvd
...
!
      ...
      do k=1,levs
        do i=1,lons_lat
          gamma(i,k)=cons1-kappa*dpp(i,k)*rpp(i,k)*cons2
          delta(i,k)=cons1+kappa*dpp(i,k)*rpp(i,k)*cons2
        enddo
      enddo
c
! ----- prepare cg and fb
...
c
      do k=1,levs
       do i=1,lons_lat
         dudt(i,k)=
     &                   - rd *tg(i,k)*dlnpx(i,k)
     &                   - dphix(i,k)-grav*zslam(i)
     &                   + vg(i,k)*coriol
         dvdt(i,k)=
     &                   - rd *tg(i,k)*dlnpy(i,k)
     &                   - dphiy(i,k)-grav*zsphi(i)
     &                   - ug(i,k)*coriol
     &                   - ek(i,k) * sinra
       enddo
      enddo
