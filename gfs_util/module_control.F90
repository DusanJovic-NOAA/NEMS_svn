...
!real(kind=kfpt),allocatable,dimension(:,:,:):: &      !nwetc,im,jm
! wg                          ! soil moisture

!real(kind=kfpt),allocatable,dimension(:,:,:):: &      !kmsc,im,jm
! tg                          ! soil temperature
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!-----------------------------------------------------------------------
...
,wcor                        ! grid separation filtering coefficient
...
!
      if(.not.global) then
        ...
        rewind nbc
        close(unit=nbc)
!       print*,'*** read tboco in consts from ',infile
        nboco=nint(tboco/dt)
      endif
!-----------------------------------------------------------------------
...
!
        ...
        fad(jds+1)=-dt/(3.*dxv(jds+1)*dyv)
        fah(jds+1)=-dt/(3.*dxv(2)*dyv*0.5*fpole)
        fcp(jds+1)= dt/(3.*cp*dxv(jds+1)*dyv*0.5*fpole)
        fdiv(jds+1)=2./(3.*dxv(jds+1)*dyv*0.5*fpole)
        hdacj(jds+1)=acdt*16.*max(dxh(jds+1),dyh) &
                /(4.*sqrt(2.)*dxv(2)*dyv*0.5*fpole)
        hdacvj(jds+1)=acdt*max(dxv(jds+1),dyv)/(4.*sqrt(2.)*dxv(jds+1)*dyv)
        ddmpu(jds+1)=cddamp*max(dxv(jds+1),dyv)/(2.*dxv(jds+1))
!-----------------------------------------------------------------------
!-------------between the poles----------------------------------------- 
!-----------------------------------------------------------------------
      write(0,*)' consts point 4 jds=',jds,' jde=',jde
        do j=jds+2,jde-2
          tph=sb+(j-jds-1)*dph
          tpv=tph+dph*0.5
          dxh(j)=a*dlm*cos(tph)
...
!       write(0,*)' in consts j=',j,' dxv=',dxv(j)
         dxv(j)=1.0*dxv(j)
          ...
          curv(j)=tan(tpv)/a
          fad(j)=-dt/(3.*dxv(j)*dyv)
          fah(j)=-dt/(3.*dxh(j)*dyh)
          fcp(j)= dt/(3.*cp*dxh(j)*dyh)
          fdiv(j)=2./(3.*dxh(j)*dyh)
          hdacj(j)= acdt*max(dxh(j),dyh)/(4.*sqrt(2.)*dxh(j)*dyh)
          hdacvj(j)=acdt*max(dxv(j),dyv)/(4.*sqrt(2.)*dxv(j)*dyv)
          ddmpu(j)=cddamp*max(dxv(j),dyv)/(2.*dxv(j))
        enddo
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
        ...
        fcp(jds)=fcp(jds+2)
        fdiv(jds)=fdiv(jds+2)
        hdacj(jds)=hdacj(jds+2)
        hdacvj(jds)=hdacvj(jds+1)
...
!-----------------------------------------------------------------------
        ...
        fcp(jde-1)= dt/(3.*cp*dxv(jde-2)*dyv*0.5*fpole)
        fdiv(jde-1)=2./(3.*dxv(jde-2)*dyv*0.5*fpole)
        hdacj(jde-1)=acdt*16.*max(dxh(jde-1),dyh) &
...
!-----------------------------------------------------------------------
        ...
        fcp(jde)=fcp(jde-2)
        fdiv(jde)=fdiv(jde-2)
        hdacj(jde)=hdacj(jde-2)
        hdacvj(jde-1)=hdacvj(jde-2)
        hdacvj(jde)=hdacvj(jde-2)
...
!-----------------------------------------------------------------------
        do j=jds,(jde-1)/2
          ...
          rdxv(j)=ave
          rdxv(jde-j)=ave
          ave=(curv(j)+curv(jde-j))*0.5
          curv(j)=ave
          curv(jde-j)=ave
          ave=(fad(j)+fad(jde-j))*0.5
          fad(j)=ave
          fad(jde-j)=ave
          ave=(hdacvj(j)+hdacvj(jde-j))*0.5
          hdacvj(j)=ave
          hdacvj(jde-j)=ave
...
!-----------------------------------------------------------------------
      ...
      write(0,*)' before f its=',its,' ite=',ite,' jts=',jts,' jte=',jte
      write(0,*)' sb=',sb,' wb=',wb,' dph=',dph,' dlm=',dlm
        tph_base=sb-dph*.5
!
        do j=jts,jte
          tph=tph_base+(j-jds+1)*dph
...
!real(kind=kfpt),dimension(1:kmsc,ims:ime,jms:jme),intent(out) :: &
! tg

!real(kind=kfpt),dimension(1:nwetc,ims:ime,jms:jme),intent(out) :: &
! wg
...
!
      ...
      allocate(tg_full(1:kmsc,ids:ide,jds:jde),stat=i)
      allocate(wg_full(1:nwetc,ids:ide,jds:jde),stat=i)
      allocate(sic(ims:ime,jms:jme),stat=i)
      allocate(stdh(ims:ime,jms:jme),stat=i)
      allocate(prra(ims:ime,jms:jme),stat=i)
      allocate(prsn(ims:ime,jms:jme),stat=i)
      allocate(prcu(ims:ime,jms:jme),stat=i)
      allocate(prec(ims:ime,jms:jme),stat=i)
      allocate(acprec(ims:ime,jms:jme),stat=i)
      allocate(cuprec(ims:ime,jms:jme),stat=i)
      allocate(cuppt(ims:ime,jms:jme),stat=i)
      allocate(insoil(ims:ime,jms:jme),stat=i)
!     allocate(cldefi(ims:ime,jms:jme),stat=i)
      allocate(inveg(ims:ime,jms:jme),stat=i)
      allocate(vegfrc(ims:ime,jms:jme),stat=i)
      allocate(kmp(ims:ime,jms:jme),stat=i)
      allocate(wliq(ims:ime,jms:jme),stat=i)
      allocate(roff(ims:ime,jms:jme),stat=i)
      allocate(epsr(ims:ime,jms:jme),stat=i)
      allocate(lpbl(ims:ime,jms:jme),stat=i)
      allocate(lbot(ims:ime,jms:jme),stat=i)
      allocate(ltop(ims:ime,jms:jme),stat=i)
      allocate(ths(ims:ime,jms:jme),stat=i)
      allocate(thz0(ims:ime,jms:jme),stat=i)
      allocate(th02(ims:ime,jms:jme),stat=i)
      allocate(qs(ims:ime,jms:jme),stat=i)
      allocate(qz0(ims:ime,jms:jme),stat=i)
      allocate(uz0(ims:ime,jms:jme),stat=i)
      allocate(vz0(ims:ime,jms:jme),stat=i)
      allocate(akhs(ims:ime,jms:jme),stat=i)
      allocate(akms(ims:ime,jms:jme),stat=i)
      allocate(ghct(ims:ime,jms:jme),stat=i)
      allocate(ustar(ims:ime,jms:jme),stat=i)
!
!-----------------------------------------------------------------------
      infile='                                                          '
!-----------------------------------------------------------------------
      ...
      enddo select_unit
!
      write(0,*)' read from unit nfcst=',nfcst,' in init'
      infile='fcst_umo'
      open(unit=nfcst,file=infile,status='old',form='unformatted')
!     read (nfcst) run,global,idat,ihrst,ihrend,ntsd
...
!     enddo
!     enddo
!     call dstrb(temp1,albedo,1,1,1,1,1)
!     call halo_exch(albedo,1,2,2)
!-----------------------------------------------------------------------
!
      if(mype==0)then
        read(2)temp1
...
!     enddo
!     enddo
!     call dstrb(temp1,sst,1,1,1,1,1)
!     call halo_exch(sst,1,2,2)
!-----------------------------------------------------------------------
!
      if(mype==0)then
        read(2)temp1
...
!     enddo
!     enddo
!     call dstrb(temp1,sno,1,1,1,1,1)
!     call halo_exch(sno,1,2,2)
!-----------------------------------------------------------------------
!
      if(mype==0)then
        read(2)temp1
...
!     enddo
!     enddo
!     call dstrb(temp1,sic,1,1,1,1,1)
!     call halo_exch(sic,1,2,2)
!-----------------------------------------------------------------------
!
      if(mype==0)then
        read(2)tg_full
...
!     enddo
!     enddo
!     call dstrb_soil(tg_full,tg,1,kmsc,1,kmsc)
!-----------------------------------------------------------------------
      if(mype==0)then
        read(2)wg_full
        close(2)
...
!     enddo
!     enddo
!     call dstrb_soil(wg_full,wg,1,nwetc,1,nwetc)
!-----------------------------------------------------------------------
      infile='sfc2_umo'
 1004 format(100i2)
!
...
!     enddo
!     enddo
!     call dstrb(temp1,vegfrc,1,1,1,1,1)
!     call halo_exch(vegfrc,1,2,2)
!-----------------------------------------------------------------------
!---cross-check/reinitialize surface data-------------------------------     
!-----------------------------------------------------------------------
!     do j=jts,jte
!       do i=its,ite
!-----------------------------------------------------------------------
...
!     enddo
!-----------------------------------------------------------------------                                                       
!---initialization of fixed land surface variables----------------------
!-----------------------------------------------------------------------                                                       
!     do j=jts,jte
!       do i=its,ite
!         z0(i,j)=0.01+stdh(i,j)*0.002+abs(fis(i,j))*0.0002
!       enddo
!     enddo
!-----------------------------------------------------------------------
 1010 format(' i,j=',i3,',',i3,' insoil=',i3,' inveg=',i3,' sst=',f6.2)
      kcfclt=0
!
!     do j=jts,jte
!       do i=its,ite
...
!         epsr(i,j)=1.
!-----------------------------------------------------------------------
!         if(sice(i,j)+sm(i,j)<0.5) then ! land
!-----------------------------------------------------------------------
!-------------standard!mix----------------------------------------------
!-----------------------------------------------------------------------
!           if(insoil(i,j)>12.or.insoil(i,j)<1) then
!             print*,'*** surface data conflict'
!             write(*,1010) i,j,insoil(i,j),inveg(i,j),sst(i,j)
!             vegfrc(i,j)=.50
!             insoil(i,j)=11
!             inveg(i,j)=7
!             albedo(i,j)=.20
!             kcfclt=kcfclt+1
!           elseif(insoil(i,j)==12) then
!             sno(i,j)=0.
!           endif
!-----------------------------------------------------------------------
!         elseif(sice(i,j)>0.5) then ! sea-ice
!-----------------------------------------------------------------------
!           sno(i,j)=0.
!           tg(kmsc,i,j)=ti0
!           epsr(i,j)=1.
...
!           inveg(i,j)=13
!-----------------------------------------------------------------------
!         elseif(sm(i,j)>0.5) then ! liquid surface
!-----------------------------------------------------------------------
!           sno(i,j)=0.
!           epsr(i,j)=1.
!
...
!       enddo
!     enddo
!     print*,'*** total conflict pts ',kcfclt 
!-----------------------------------------------------------------------
!     do j=jts,jte
!       do i=its,ite
!-----------------------------------------------------------------------
!---initialize!sub-surface temperature and moisture---------------------
!-----------------------------------------------------------------------
!         if(sm(i,j)<0.5) then ! land and sea-ice surface
!-----------------------------------------------------------------------
!           kmp(i,j)=nosnoc
!           if(insoil(i,j)==10) then
!             do k=1,nwetc
!               wg(k,i,j)=0.0
!             enddo
!           endif
...
!-----------------------------------------------------------------------
!       enddo
!     enddo ! end of land surface initialization
!-----------------------------------------------------------------------
!     do j=jts,jte
!       do i=its,ite
!         lpbl(i,j)=lm
...
!         if(sm(i,j)<0.5) then
!           ths(i,j)=t(i,j,lm) &
!                   *(100000./(sgml2(lm)*pd(i,j)+psgml1(lm)))**cappa
!         else
!           ths(i,j)=sst(i,j)*(100000.0/(pd(i,j)+pdtop+pt))**cappa
!         endif
!
!         thz0(i,j)=ths(i,j)
!         th02(i,j)=ths(i,j)
