...
!
      ...
      USE atmos_grid_comp_mod,only: atmos_setservices
!
!-----------------------------------------------------------------------
...
!
      ...
      CALL ESMF_GridCompSetServices(gc_atm(i)                            &  !<-- The ATM gridded component
                                   ,atmos_setservices                         &  !<-- User's subroutineName
                                   ,RC)
      end do
      ENDIF
!
...
!
      CALL ESMF_TimeIntervalSet(timeinterval=TIMESTEP                   &  !<-- Dummy fundamental timestep (sec) (ESMF)
                               ,s           =TIMESTEP_SEC_WHOLE         &  !<-- Dummy whole seconds
                               ,sn          =TIMESTEP_SEC_NUMERATOR     &  !<-- Dummy numerator of timestep fraction

                               ,sd          =TIMESTEP_SEC_DENOMINATOR   &  !<-- Dummy denominator of timestep fraction
                               ,rc          =RC)
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
      CALL ERR_MSG(RC,MESSAGE_CHECK,RC_MAIN)
