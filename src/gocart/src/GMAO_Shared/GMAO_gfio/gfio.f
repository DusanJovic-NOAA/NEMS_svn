...
!        dimId = ncvid (fid, dimName, rc)
        rc = nf_inq_dimid (fid, dimName, dimId)
...
!        dimId = ncvid (fid, dimName, rc)
        rc = nf_inq_dimid (fid, dimName, dimId)
        if (err("DimInquire: ncvid failed",rc,-40) .NE. 0) return
        call ncagtc (fid, dimId, 'units', dimUnits, MAXCHR, rc)
...
! Local Variables
      ...
      integer timeId, latId, lonId, levId, dimId, incSecs, incTime
...
! Make NetCDF errors non-fatal, and DO NOT issue warning messages.
      ...
      call ncpopt(0) ! ask netcdf to shut up
...
!        dimId = ncvid (fid, dimName, rc)
        rc = nf_inq_dimid (fid, dimName, dimId)
        if (err("Inqure: ncvid failed",rc,-40) .NE. 0) return
        call ncagtc (fid, dimId, 'units', dimUnits, MAXCHR, rc)
