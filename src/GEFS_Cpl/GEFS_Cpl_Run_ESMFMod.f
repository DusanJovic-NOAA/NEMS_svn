...
!------------------------------------------
 cst%CENTER = 1
 CALL GEFS_Sto_Per_Scheme_Step1(cst)
 ...
 IF(ESMF_LogMsgFoundError(rc1, "Distribute Back For the step2 - CLWM")) THEN
     rcfinal = ESMF_FAILURE
...
!------------------------------------------
 ...
 CALL GEFS_Sto_Per_Scheme_Step2(cst, 1, 2, Jul_Day, 0.34, -0.34, rc1)

 PRINT*, 'Finished calling STEP2_2'
 PRINT*, 'Start calling STEP2_1'

 CALL GEFS_Sto_Per_Scheme_Step2(cst, 1, 1, Jul_Day, 0.34, -0.34, rc1)
...
!---------------------------------------------------------
 CALL GEFS_Sto_Per_Scheme_Step1_2(impState, cst, rc1)

 IF(ESMF_LogMsgFoundError(rc1, "Run Sto_Per_Scheme_Step1_2")) THEN
     rcfinal = ESMF_FAILURE
 ...
 IF(rcfinal == ESMF_SUCCESS) THEN
     PRINT*, "PASS: GEFS_Cpl_Run."
 ELSE
     PRINT*, "FAIL: GEFS_Cpl_Run."
 END IF
