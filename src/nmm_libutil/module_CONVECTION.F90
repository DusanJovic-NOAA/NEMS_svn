...
!
      PRIVATE
!
      PUBLIC :: BMJ_INIT,CUCNVC
!
!-----------------------------------------------------------------------
!
      INTEGER :: MYPE
!
      REAL :: XLS=ELIV,XLV=ELWV
!
      REAL :: EP_1,SVPT0  ! These appear in the WRF cumulus driver
!
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
...
!
     ...
     &                 ,EPSUP=1.00                                      &
     &                 ,fres=1.20,FR=1.00,FSL=0.85,FSS=0.85             &
     &                 ,FUP=1./200000.                                  &
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!***  FOR GRELL_DEVENYI CONVECTION
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!
      INTEGER,PARAMETER :: ENSDIM=144
!
!-----------------------------------------------------------------------
!
      CONTAINS
...
!-----------------------------------------------------------------------
!
      INTEGER :: CU_PHYSICS,I,ICLDCK,J,K,KFLIP,MNTO                     &
                ,N,NCUBOT,NCUTOP,NSTEP_CNV,N_TIMSTPS_OUTPUT
!
...
!
!jaa      REAL,DIMENSION(1:LM) :: QL,TL
      REAL  :: QL,TL
!
      REAL,DIMENSION(IMS:IME,JMS:JME) :: CUBOT,CUTOP,GSW,NCA            &
                                        ,RAINC,RAINCV,SFCZ,XLAND
!
      REAL,DIMENSION(IMS:IME,1:LM+1) :: WMID,Z
!
      REAL,DIMENSION(IMS:IME,1:LM+1,JMS:JME) :: DZ,P8W,P_PHY,PI_PHY     &
                                               ,RQCCUTEN,RQRCUTEN       &
                                               ,RQICUTEN,RQSCUTEN       &
...
!.......................................................................
!$omp parallel do                                                       &
!$omp& private(j,i,k,pdsl,kflip,plyr,ql,tl)
!.......................................................................
      DO J=JTS,JTE
      DO I=ITS,ITE
!
...
!***  THE GROUND.
!
        KPBL(I,J)=LM-LPBL(I,J)+1
!
!-----------------------------------------------------------------------
!***  FILL VERTICAL WORKING ARRAYS.
!***  THE FUNDAMENTAL UMO ARRAYS NEED TO FLIP THEIR DATA
!***  BEFORE FOR SENDING IT TO THE WRF PHYSICS DRIVERS.
!***  USE THE FLIPPED INDEX IN THE UMO ARRAYS.
!-----------------------------------------------------------------------
!
        DO K=1,LM
          KFLIP=LM+1-K
!
          ...
          TL=T(I,J,KFLIP)
          RR(I,K,J)=PLYR/(R_D*TL*(P608*ql+1.))
          T_PHY(I,K,J)=TL
!
          TH_PHY(I,K,J)=TL*(1.E5/PLYR)**CAPA
...
!.......................................................................
!$omp parallel do                                                       &
!$omp private(i,j,k,pdsl,plyr,kflip,ql_k,wmid)
!.......................................................................
      DO J=JTS,JTE
        DO I=ITS,ITE
          WMID(I,1)=-OMGALF(I,J,LM)*CP/(G*DT)
          PDSL=PD(I,J)
          PLYR=PSGML1(LM)+SGML2(LM)*PDSL+PT
          DZ(I,1,J)=T(I,J,LM)*(P608*Q(I,J,LM)+1.)*R_D                   &
                    *(P8W(I,1,J)-P8W(I,2,J))                            &
                    /(PLYR*G)
        ENDDO
!
        ...
        DO I=ITS,ITE
          KFLIP=LM+1-K
          QL_K=MAX(Q(I,J,KFLIP),EPSQ)
          WMID(I,K)=-OMGALF(I,J,KFLIP)*CP/(G*DT)
          WINT(I,K,J)=0.5*(WMID(I,K-1)+WMID(I,K))
          DZ(I,K,J)=T_PHY(I,K,J)*(P608*QL_K+1.)*R_D                     &
...
!-----------------------------------------------------------------------
!
      IF(CU_PHYSICS/=BMJSCHEME)THEN
!
!jaa!$omp parallel do                                                       &
!jaa!$omp& private(i,j,k,kflip)
        DO J=JTS_B1_H1,JTE_B1_H1
!
          DO K=1,LM
            KFLIP=LM+1-K
            DO I=ITS_B1_H1,ITE_H1
...
!
!.......................................................................
!$omp parallel do private(n,j,k,i,kflip)
!.......................................................................
      DO N=1,NUM_WATER
        DO K=1,LM
        KFLIP=LM+1-K
...
!.......................................................................
!$omp parallel do                                                       &
!$omp& private(j,k,i,kflip,dqdt,dtdt,tchange,pcpcol,ncubot,ncutop)
!.......................................................................
!-----------------------------------------------------------------------
      DO J=JTS,JTE
      DO I=ITS,ITE
...
!
          DQDT=RQVCUTEN(I,K,J)/(1.+WATER_TRANS(I,K,J,P_QV))**2
!
!***  RTHCUTEN IN BMJDRV IS DTDT OVER PI.
!
          DTDT=RTHCUTEN(I,K,J)*PI_PHY(I,K,J)
          T(I,J,KFLIP)=T(I,J,KFLIP)+DTDT*DTCNVC
          Q(I,J,KFLIP)=Q(I,J,KFLIP)+DQDT*DTCNVC
...
!.......................................................................
      DO N=2,NUM_WATER
        ...
        KFLIP=LM+1-K
        DO I=IMS,IME
          WATER(I,J,K,N)=WATER_TRANS(I,KFLIP,J,N)
        ENDDO
        ENDDO
        ENDDO
      ENDDO
...
!jaa      !$omp parallel do   &
!jaa      !$omp private ( ij ,its,ite,jts,jte, i,j,k),SCHEDULE(dynamic)
...
!***  End debugging convection
      ...
      IF(ICLDCK==0.OR.ITIMESTEP==0)THEN
!
        DO J=JTS,JTE
        DO I=ITS,ITE
...
!.......................................................................
!$omp parallel do &
!$omp private (j,i,dqdt,dtdt,pcpcol,psfc,ptop,landmask,k,kflip,qcol   &
...
!-----------------------------------------------------------------------
      ...
      ENDIF
!-----------------------------------------------------------------------
!***  New cloud at swap points
!-----------------------------------------------------------------------
!all points               go to 810 ! all points, new cloud
!swap points               if(iswap.gt.0) go to 810 ! swap points, new cloud
!-----------------------------------------------------------------------
!***  Otherwise try with old cloud
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
                 if(dst.gt.0.) go to 810 ! new cloud
!-----------------------------------------------------------------------
!      if(dst.gt.0.) then ! swaponly
!        lbot=0           ! swaponly
!!!!!    ltop=lbot        ! swaponly
!        ltop=kte         ! swaponly
!        ptop=pbot        ! swaponly
