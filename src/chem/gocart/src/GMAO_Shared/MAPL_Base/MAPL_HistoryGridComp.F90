...
! $Id: MAPL_HistoryGridComp.F90,v 1.12.4.7.2.1.10.1 2009/05/14 19:23:38 dasilva Exp $

#include "MAPL_Generic.h"

#define nsecf(nhms)  (nhms/10000*3600 + mod(nhms,10000)/100*60 + mod(nhms,100))
...
! create XFORM
      ...
      do m=1,list(n)%nfield
         ...
         if (internal_state%average(n)) then
            call ESMF_StateAdd(internal_state%CIM(N), f, rc=status)
            VERIFY_(STATUS)
            ...
            VERIFY_(STATUS)
            call ESMF_StateAdd(internal_state%GIM(N), f, rc=status)
            VERIFY_(STATUS)
...
! add field to state_out
            call ESMF_StateAdd(internal_state%Regrid(N)%PTR%state_out, &
                 field, rc=status)
            VERIFY_(STATUS)
         endif
...
! CFIO
    do n=1,nlist
       ...
       if (list(n)%format == 'CFIO') then
          ...
          do m=1,list(n)%nfield
             call ESMF_StateGet( INTERNAL_STATE%GIM(n), &
...
!ALT: unit = -1 is currently "used" to indicate CFIO 
         ...
         if (list(n)%unit == -1) then 
            ...
            call MAPL_CFIOWrite( MCFIO=list(n)%MCFIO,                 &
                 CLOCK=clock, BUNDLE=list(n)%bundle, NBITS=list(n)%nbits,  &
                 RC=status )

            VERIFY_(STATUS)
            if ( nsecf( list(n)%frequency ) == nsecf( list(n)%duration )) then
                call MAPL_CFIODestroy (list(n)%mcfio, rc=status)
                VERIFY_(STATUS)
                list(n)%unit = 0
            end if
