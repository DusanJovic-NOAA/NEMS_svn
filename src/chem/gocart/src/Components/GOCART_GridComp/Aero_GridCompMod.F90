...
! !USES:
   ...
   use CARMA_GridCompMod     ! CARMA Service Component
...
!  CARMA
!  -----
   if ( w_c%reg%doing_CARMA ) then
      call CARMA_GridCompInitialize ( gcThis%gcCARMA, w_c, impChem, expChem, &
                                      nymd, nhms, cdt, rc )
      if ( rc /= 0 ) then  
           if (MAPL_AM_I_ROOT()) print *, Iam//': CARMA failed to initialize ', rc
           rc = 9000 + rc
           return
      end if
   end if
...
!  -----
   if ( w_c%reg%doing_CARMA ) then
      call CARMA_GridCompRun ( gcThis%gcCARMA, w_c, impChem, expChem, &
                               nymd, nhms, cdt, rc )
      if ( rc /= 0 ) then  
           rc = 9000 + rc
           return
...
!  -----
   if ( w_c%reg%doing_CARMA ) then
      call CARMA_GridCompFinalize ( gcThis%gcCARMA, w_c, impChem, expChem, &
                                    nymd, nhms, cdt, rc )
      if ( rc /= 0 ) then  
           rc = 9000 + rc
           return
