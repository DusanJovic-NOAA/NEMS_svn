...
!========================================!
      module module_radiation_driver_gfs !
!........................................!
!
      ...
      use module_radiation_astronomy_gfs,only : solinit
      use module_radiation_gases_gfs,   only : NF_VGAS, getgases, getozn,   &
     &                                     gasinit
      use module_radiation_aerosols_gfs,only : NF_AESW, aerinit, setaer,    &
     &                                     NF_AELW, laswflg,            &
...
! --- add nv_aod for aerosol diag (Sarah Lu)
     &                                     nv_aod
      use module_radiation_surface_gfs, only : NF_ALBD, sfcinit, setalb,    &
     &                                     setemis
      use module_radiation_clouds_gfs,  only : NF_CLDS, cldinit,            &
...
!
      return
!...................................
      end subroutine radinit_gfs
!-----------------------------------


!-----------------------------------
      subroutine grrad_gfs                                              &
!...................................
...
!  --- ...  compute relative humidity
      ...
      do k = 1, LM
        do i = 1, IM
          es  = min( prsl(i,k), fpvs( tgrs(i,k) ) )   ! fpvs in pa
...
!         plyr(i,k) = 10.0 * prsl(i,k)
          plvl(i,k) = 0.01 * prsi(i,k)
          plyr(i,k) = 0.01 * prsl(i,k)
          tlyr(i,k) = tgrs(i,k)
...
!  ---  outputs:
     ...
     &      )
        ...
        elseif (np3d == 5) then          ! nmmb ferrier+bmj

!     print *,' in grrad : calling progcld3'
!         call progcld3                                                 &
!  ---  inputs:
!    &     ( plyr,plvl,tlyr,qlyr,qstl,rhly,clw,                         &
!    &       xlat,xlon,slmsk, fcice,frain,rrime,flgmin,                 &
...
!
      return
!...................................
      end subroutine grrad_gfs
!-----------------------------------


!
!........................................!
      end module module_radiation_driver_gfs !
!========================================!
