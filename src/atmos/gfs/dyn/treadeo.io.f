...
!      REAL(KIND=KIND_EVOD),intent(in) :: PLNOD_R(LEN_TRIE_LS,LATR2)
!      REAL(KIND=KIND_EVOD),intent(in) :: PLNEW_R(LEN_TRIE_LS,LATR2)
!      REAL(KIND=KIND_EVOD),intent(in) :: PLNOW_R(LEN_TRIE_LS,LATR2)
...
!------local vars
!
      INTEGER              NTRACI, ntozi, ntcwi, ncldi, ixgr
     &,                    nt0, direction,levsi
!
...
!    &,                    SI(LEVP1)
     &,                    Z00
!jw,physics part      real (kind=kind_phys)
!jw      real (kind=kind_evod)
!jw     &     phy_f3d(LONR,LEVS,num_p3d,lats_node_r),
!jw     &     phy_f2d(lonr,num_p2d,lats_node_r)
!
!jw      real(kind=kind_evod) syn_gr_a_1(lonfx*lots,lats_dim_a)
!jw      real(kind=kind_evod) syn_gr_a_2(lonfx*lots,lats_dim_a)
...
!
!jw      integer kmsk(lonf,latg)
!jw      real(kind=kind_ior), target ::  buff1(lonr*latr)
!jw      real(kind=kind_io8) buffo(lonr,lats_node_r)
!jw     &,                   buff2(lonr,lats_node_r)
...
!jw      DO LOCL=1,LS_MAX_NODE
!jw              L=ls_node(locl,1)
!jw         jbasev=ls_node(locl,2)
!jw         indev1 = indlsev(L,L)
!jw         if (mod(L,2).eq.mod(jcap+1,2)) then
!jw            indev2 = indlsev(jcap+1,L)
!jw         else
!jw            indev2 = indlsev(jcap  ,L)
!jw         endif
!jw         do indev = indev1 , indev2
!jw            GZE(INDEV,1)=
!jw     X      GZE(INDEV,1)*SNNP1EV(INDEV)*GA2
!jw            GZE(INDEV,2)=
!jw     X      GZE(INDEV,2)*SNNP1EV(INDEV)*GA2
!jw         END DO
!jw      END DO
!jw      DO LOCL=1,LS_MAX_NODE
!jw              L=ls_node(locl,1)
!jw         jbasod=ls_node(locl,3)
!jw         indod1 = indlsod(L+1,L)
!jw         if (mod(L,2).eq.mod(jcap+1,2)) then
!jw            indod2 = indlsod(jcap  ,L)
!jw         else
!jw            indod2 = indlsod(jcap+1,L)
!jw         endif
!jw         do indod = indod1 , indod2
!jw            GZO(INDOD,1)=
!jw     X      GZO(INDOD,1)*SNNP1OD(INDOD)*GA2
!jw            GZO(INDOD,2)=
!jw     X      GZO(INDOD,2)*SNNP1OD(INDOD)*GA2
!jw         END DO
!jw      END DO
...
!
      do lan=1,lats_node_a
        ...
        do k=1,levs
          do i=1,lons_lat
            grid_gr(i,lan,G_uum+k-1)=
     &         syn_gr_a_2(i+(ksu -2+k)*lon_dim,lan)
            grid_gr(i,lan,G_vvm+k-1)=
     &         syn_gr_a_2(i+(ksv -2+k)*lon_dim,lan)
            grid_gr(i,lan,G_ttm+k-1)=
     &         syn_gr_a_2(i+(kst -2+k)*lon_dim,lan)
            grid_gr(i,lan,G_dpm+k-1)=
     &         syn_gr_a_2(i+(ksdp-2+k)*lon_dim,lan)
          enddo
        enddo
        do i=1,lons_lat
            grid_gr(i,lan,G_qm)= 
...
!     &   maxval(grid_gr(:,1:lats_node_a,g_qm)),
!     &   minval(grid_gr(:,1:lats_node_a,g_qm)),'gz=',
!     &   maxval(grid_gr(:,1:lats_node_a,g_gz)),
!     &   minval(grid_gr(:,1:lats_node_a,g_gz)),
!     &   '1:lats_node_a=',lats_node_a
