      subroutine sicdife_hyb_gcdp(de,te,qe,xe,ye,ze,de_n,te_n,qe_n,
     x                                              dti,ue,ve,
     x                       ls_node,snnp1ev,ndexev,locl)

!
! semi-implicit for thickness in p, divergence, and enthalpy
! by hann-ming henry juang 2008
!
! program log
! 2011 02 20 : henry juang, add dp for mass_dp option
!
     ...
     x                       + de_n(indev,2,k+1)*bmhyb(j,k+1)
      ...
      do j=1,levs
         do k=1,levs,2
            do indev = indev1 , indev2
               ze(indev,1,j) =
     x         ze(indev,1,j) + de_n(indev,1,k  )*smhyb(j,k  )
     x                       + de_n(indev,1,k+1)*smhyb(j,k+1)
               ze(indev,2,j) =
     x         ze(indev,2,j) + de_n(indev,2,k  )*smhyb(j,k  )
     x                       + de_n(indev,2,k+1)*smhyb(j,k+1)
            enddo
         enddo
      enddo
      ...
      do k=1,levs
         do indev = indev1 , indev2
            qdtze(indev,1,k)   =  qe(indev,1,k)-qe_n(indev,1,k)
     x                     + dt*ze(indev,1,k)
 
            qdtze(indev,2,k)   =  qe(indev,2,k)-qe_n(indev,2,k)
     x                   +   dt*ze(indev,2,k)
         enddo
      enddo
...
c$$$               ue(indev,1,j) =
c$$$     x         ue(indev,1,j) + qdtze(indev,1,k  )*hmhyb(j,k  )
c$$$     x                       + qdtze(indev,1,k+1)*hmhyb(j,k+1)
c$$$               ue(indev,2,j) =
c$$$     x         ue(indev,2,j) + qdtze(indev,2,k  )*hmhyb(j,k  )
c$$$     x                       + qdtze(indev,2,k+1)*hmhyb(j,k+1)
c$$$            enddo
c$$$         enddo
c$$$      enddo
     ...
     &               ve(indev1,2,1), len_trie_ls*2)
         call dgemm ('n', 't',
     &               indev2-indev1+1, levs, levs, cons1,
     &               qdtze(indev1,1,1), len_trie_ls*2,
     &               hmhyb(1,1), levs, cons0,
     &               ue(indev1,1,1), len_trie_ls*2)
         call dgemm ('n', 't',
     &               indev2-indev1+1, levs, levs, cons1,
     &               qdtze(indev1,2,1), len_trie_ls*2,
     &               hmhyb(1,1), levs, cons0,
     ...
     &               ue(indev1,2,1), len_trie_ls*2)
      ...
      do j=1,levs
            do indev = indev1 , indev2
               ve(indev,1,j) =
     x         ve(indev,1,j) +           ue(indev,1,j)
 
               ve(indev,1,j) =
     x         ve(indev,1,j) *      snnp1ev(indev)
 
               ve(indev,2,j) =
     x         ve(indev,2,j) +           ue(indev,2,j)
...
c$$$            do indev = indev1 , indev2
c$$$               ue(indev,1,j) =
c$$$     x         ue(indev,1,j) + ve(indev,1,k  )*smhyb(j,k  )
c$$$     x                       + ve(indev,1,k+1)*smhyb(j,k+1)
c$$$               ue(indev,2,j) =
c$$$     x         ue(indev,2,j) + ve(indev,2,k  )*smhyb(j,k  )
c$$$     x                       + ve(indev,2,k+1)*smhyb(j,k+1)
c$$$            enddo
c$$$         enddo
c$$$      enddo
     ...
     &               ue(indev1,2,1), len_trie_ls*2)
         ...
         do k=1,levs
         do indev = indev1 , indev2
            qdtze(indev,1,k)   = qdtze(indev,1,k) +
     x       qe_n(indev,1,k)   - dt*ue(indev,1,k)

               ze(indev,1,k)   = repsp*qdtze(indev,1,k)  !constant
     x                       - repsm*   qe(indev,1,k)
 
            qdtze(indev,2,k)   = qdtze(indev,2,k) +
     x       qe_n(indev,2,k)   - dt*ue(indev,2,k)

               ze(indev,2,k)   = repsp*qdtze(indev,2,k)  !constant
     x                       - repsm*   qe(indev,2,k)
         enddo
         enddo
c$$$      do j=1,levs
c$$$            do indev = indev1 , indev2
...
!     print *,' leave sicdife_hyb_gc_fd '		! hmhj
      ...
      subroutine sicdifo_hyb_gcdp(do,to,qo,xo,yo,zo,do_n,to_n,qo_n,
     x                                              dti,uo,vo,
     ...
     x                       + do_n(indod,2,k+1)*bmhyb(j,k+1)
      ...
      do j=1,levs
         do k=1,levs,2
            do indod = indod1 , indod2
               zo(indod,1,j) =
     x         zo(indod,1,j) + do_n(indod,1,k  )*smhyb(j,k  )
     x                       + do_n(indod,1,k+1)*smhyb(j,k+1)
               zo(indod,2,j) =
     x         zo(indod,2,j) + do_n(indod,2,k  )*smhyb(j,k  )
     x                       + do_n(indod,2,k+1)*smhyb(j,k+1)
            enddo
         enddo
      enddo
      ...
      do k=1,levs
         do indod = indod1, indod2
            qdtzo(indod,1,k)   =  qo(indod,1,k)-qo_n(indod,1,k)
     x                   +   dt*zo(indod,1,k)
 
            qdtzo(indod,2,k)   =  qo(indod,2,k)-qo_n(indod,2,k)
     x                   +   dt*zo(indod,2,k)
         enddo
      enddo
...
c$$$               uo(indod,1,j) =
c$$$     x         uo(indod,1,j) + qdtzo(indod,1,k  )*hmhyb(j,k  )
c$$$     x                       + qdtzo(indod,1,k+1)*hmhyb(j,k+1)
c$$$               uo(indod,2,j) =
c$$$     x         uo(indod,2,j) + qdtzo(indod,2,k  )*hmhyb(j,k  )
c$$$     x                       + qdtzo(indod,2,k+1)*hmhyb(j,k+1)
c$$$            enddo
c$$$         enddo
c$$$      enddo
      if ( kind_evod .eq. 8 ) then !------------------------------------
     ...
     &               vo(indod1,2,1), len_trio_ls*2)
         call dgemm ('n', 't',
     &               indod2-indod1+1, levs, levs, cons1,
     &               qdtzo(indod1,1,1), len_trio_ls*2,
     &               hmhyb(1,1), levs, cons0,
     &               uo(indod1,1,1), len_trio_ls*2)
         call dgemm ('n', 't',
     &               indod2-indod1+1, levs, levs, cons1,
     &               qdtzo(indod1,2,1), len_trio_ls*2,
     &               hmhyb(1,1), levs, cons0,
     ...
     &               uo(indod1,2,1), len_trio_ls*2)
      ...
      do j=1,levs
            do indod = indod1, indod2
               vo(indod,1,j) =
     x         vo(indod,1,j) +           uo(indod,1,j)
 
               vo(indod,1,j) =
     x         vo(indod,1,j) *      snnp1od(indod)
 
               vo(indod,2,j) =
     x         vo(indod,2,j) +           uo(indod,2,j)
...
c$$$            do indod = indod1 , indod2
c$$$               uo(indod,1,j) =
c$$$     x         uo(indod,1,j) + vo(indod,1,k  )*smhyb(j,k  )
c$$$     x                       + vo(indod,1,k+1)*smhyb(j,k+1)
c$$$               uo(indod,2,j) =
c$$$     x         uo(indod,2,j) + vo(indod,2,k  )*smhyb(j,k  )
c$$$     x                       + vo(indod,2,k+1)*smhyb(j,k+1)
c$$$            enddo
c$$$         enddo
c$$$      enddo
      if ( kind_evod .eq. 8 ) then !------------------------------------
     ...
     &               uo(indod1,2,1), len_trio_ls*2)
         ...
         do k=1,levs
         do indod = indod1, indod2
            qdtzo(indod,1,k)   = qdtzo(indod,1,k) +
     x       qo_n(indod,1,k)   - dt*uo(indod,1,k)

               zo(indod,1,k)   = repsp*qdtzo(indod,1,k)  !constant
     x                       - repsm*   qo(indod,1,k)
 
            qdtzo(indod,2,k)   = qdtzo(indod,2,k) +
     x       qo_n(indod,2,k)   - dt*uo(indod,2,k)

               zo(indod,2,k)   = repsp*qdtzo(indod,2,k)  !constant
     x                       - repsm*   qo(indod,2,k)
         enddo
         enddo
c$$$      do j=1,levs
c$$$            do indod = indod1 , indod2
