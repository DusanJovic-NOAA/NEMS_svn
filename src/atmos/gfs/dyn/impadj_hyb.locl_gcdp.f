      subroutine impadje_hyb_gcdp(de,te,qe,xe,ye,ze,
     x                       dti,ue,ve,snnp1ev,ndexev,
     x                       ls_node,locl)

!
! program log
! 20110220:    Henry Juang add this routine for MASS_DP and NDSLFV
!
...
!     eps_si=0.20
      ...
      do k=1,levs
        ...
        do j=1,levs 
          do indev = indev1 , indev2
            ue(indev,1,k)=ue(indev,1,k)+hmhyb(k,j)*ze(indev,1,j)
            ue(indev,2,k)=ue(indev,2,k)+hmhyb(k,j)*ze(indev,2,j)
          enddo
        enddo 

          do indev = indev1 , indev2
            ue(indev,1,k)=xe(indev,1,k)
     &     +dt*snnp1ev(indev)*(ve(indev,1,k)+ue(indev,1,k))
            ue(indev,2,k)=xe(indev,2,k)
     &     +dt*snnp1ev(indev)*(ve(indev,2,k)+ue(indev,2,k))
          enddo
     ...
     x      ve(indev,2,k) +dm205_hyb(ndexev(indev)+1,k,j)*ue(indev,2,j)
      ...
      do j=1,levs
       do k=1,levs 
        do indev = indev1 , indev2
          qe(indev,1,j)=qe(indev,1,j)-dt*smhyb(j,k)*ve(indev,1,k)
          qe(indev,2,j)=qe(indev,2,j)-dt*smhyb(j,k)*ve(indev,2,k)
        enddo
       enddo 
      enddo
      ...
      do j=1,levs
        do indev = indev1 , indev2
          qe(indev,1,j)=qe(indev,1,j)+ze(indev,1,j)
          qe(indev,2,j)=qe(indev,2,j)+ze(indev,2,j)
        enddo
      enddo
...
!     print *,' leave impadje_hyb.locl_gc_fd '		! hmhj
      ...
      subroutine impadjo_hyb_gcdp(do,to,qo,xo,yo,zo,
...
!     eps_si=0.20
      ...
      do k=1,levs
        ...
        do j=1,levs
          do indod = indod1 , indod2
            uo(indod,1,k)=uo(indod,1,k)+hmhyb(k,j)*zo(indod,1,j)
            uo(indod,2,k)=uo(indod,2,k)+hmhyb(k,j)*zo(indod,2,j)
          enddo
        enddo

          do indod = indod1 , indod2
            uo(indod,1,k)=xo(indod,1,k)
     &     +dt*snnp1od(indod)*(vo(indod,1,k)+uo(indod,1,k))
            uo(indod,2,k)=xo(indod,2,k)
     &     +dt*snnp1od(indod)*(vo(indod,2,k)+uo(indod,2,k))
          enddo
...
!    x      vo(indod,2,k) +d_hyb_m(k,j,ndexod(indod)+1)*uo(indod,2,j)
      ...
      do j=1,levs
       do k=1,levs 
        do indod = indod1 , indod2
          qo(indod,1,j)=qo(indod,1,j)-dt*smhyb(j,k)*vo(indod,1,k)
          qo(indod,2,j)=qo(indod,2,j)-dt*smhyb(j,k)*vo(indod,2,k)
        enddo
       enddo 
      enddo
      do j=1,levs 
        do indod = indod1 , indod2
          qo(indod,1,j)=qo(indod,1,j)+zo(indod,1,j)
          qo(indod,2,j)=qo(indod,2,j)+zo(indod,2,j)
        enddo
      enddo 
