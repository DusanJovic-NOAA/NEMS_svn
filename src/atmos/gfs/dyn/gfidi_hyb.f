     ...
     &  dg,tg,zg,ug,vg,rqg,dphi,dlam,qg,zsphi,zslam,
...
! hmhj add
      do k=1,levs
      do j=1,lons_lat
        dudt(j,levs+1-k)=dudt(j,levs+1-k)-grav*zslam(j)
        dvdt(j,levs+1-k)=dvdt(j,levs+1-k)-grav*zsphi(j)
      enddo
      enddo
!..............................................................
      do k=1,levs
...
!
      ...
      do 339 n=1,ntrac
        ...
        do k=1,levm1            !k=1, top
         rqg_half(j,k,n)=cons0p5*(rqg(j,levs-k,n)+rqg(j,levs+1-k,n))
        enddo
        ...
        enddo
        if(rqg(j,levs,n).ge.cons0) then
         rqg_d(j,0,n)=rqg(j,levs,n)-
     1     max(cons0,cons2*rqg(j,levs,n)-rqg(j,levs-1,n))
        else
         rqg_d(j,0,n)=rqg(j,levs,n)-
     1     min(cons0,cons2*rqg(j,levs,n)-rqg(j,levs-1,n))
        endif
        if(rqg(j,1,n).ge.cons0) then
...
!   (without this update, the scheme is the same as that in loop 340)
        do j=1,lons_lat
        do k=1,levs-1
        if(dot(j,k+1).gt.cons0) then            !dot is from top to bottom
         rrkp=cons0
         if(rqg_d(j,k,n).ne.cons0) rrkp=rqg_d(j,k-1,n)/rqg_d(j,k,n)
         phkp=(rrkp+abs(rrkp))/(1+abs(rrkp))
         rqg_half(j,k,n)=rqg(j,levs+1-k,n)+
     1                   phkp*(rqg_half(j,k,n)-rqg(j,levs+1-k,n))
        else
         rrk1m=cons0
         if(rqg_d(j,k,n).ne.cons0) rrk1m=rqg_d(j,k+1,n)/rqg_d(j,k,n)
         phk1m=(rrk1m+abs(rrk1m))/(1+abs(rrk1m))
         rqg_half(j,k,n)=rqg(j,levs-k,n)+
     1                   phk1m*(rqg_half(j,k,n)-rqg(j,levs-k,n))
        endif
        enddo
        enddo
        ...
        do j=1,lons_lat
        do k=1,levs
         bb=rqg_half(j,k,n)*dot(j,k+1)-rqg_half(j,k-1,n)*dot(j,k)
         cc=-rqg(j,levs+1-k,n)*(dot(j,k+1)-dot(j,k))
         tmpdrdt=-rdel(j,k)*(bb+cc)
         if(zfirst) then
          zadv(j,levs+1-k,3+n)=tmpdrdt
         else
          zadv(j,levs+1-k,3+n)=cons0p5*(tmpdrdt+szdrdt(j,levs+1-k,n))
         endif
         szdrdt(j,levs+1-k,n)=tmpdrdt
        enddo
        enddo
339   continue
!     --------------------
      ...
      do j=1,lons_lat
        zadv(j,levs+1-k,3+n)=
     1 -rdel2(j,k)*dot(j,k+1)*( rqg(j,levs-k,n)-rqg(j,levs+1-k,n) )
      enddo
 
      k=levs
      do j=1,lons_lat
        zadv(j,levs+1-k,3+n)=
     1 -rdel2(j,k)*dot(j,k)*( rqg(j,levs+1-k,n)-rqg(j,levs+2-k,n) )
      enddo
 
      do k=2,levm1
      do j=1,lons_lat
        zadv(j,levs+1-k,3+n)=-rdel2(j,k)*
     1 ( dot(j,k+1)*( rqg(j,levs  -k,n)-rqg(j,levs+1-k,n) ) +
     2   dot(j,k  )*( rqg(j,levs+1-k,n)-rqg(j,levs+2-k,n) ) )
      enddo
      enddo
340   continue
!     --------------------
