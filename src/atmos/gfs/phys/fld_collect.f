...
!collect data 
      do j=1,lats_node_r_max
        do i=1,lonr
          ...
          grid_node(i,j,2) = sfc_fld%sheleg(i,j)
          grid_node(i,j,3) = sfc_fld%tg3(i,j)
          grid_node(i,j,4) = sfc_fld%zorl(i,j)
!          grid_node(i,j,5) = sfc_fld%cv(i,j)
!         grid_node(i,j,6) = sfc_fld%cvb(i,j)
!         grid_node(i,j,7) = sfc_fld%cvt(i,j)
          grid_node(i,j,5) = sfc_fld%alvsf(i,j)
          grid_node(i,j,6) = sfc_fld%alvwf(i,j)
          grid_node(i,j,7) = sfc_fld%alnsf(i,j)
          grid_node(i,j,8) = sfc_fld%alnwf(i,j)
          grid_node(i,j,9) = sfc_fld%slmsk(i,j)
          grid_node(i,j,10) = sfc_fld%vfrac(i,j)
          grid_node(i,j,11) = sfc_fld%canopy(i,j)
          grid_node(i,j,12) = sfc_fld%f10m(i,j)
!          grid_node(i,j,16) = sfc_fld%t2m(i,j)
!          grid_node(i,j,17) = sfc_fld%q2m(i,j)
          grid_node(i,j,13) = sfc_fld%vtype(i,j)
          grid_node(i,j,14) = sfc_fld%stype(i,j)
          grid_node(i,j,15) = sfc_fld%facsf(i,j)
          grid_node(i,j,16) = sfc_fld%facwf(i,j)
          grid_node(i,j,17) = sfc_fld%uustar(i,j)
          grid_node(i,j,18) = sfc_fld%ffmm(i,j)
          grid_node(i,j,19) = sfc_fld%ffhh(i,j)
          grid_node(i,j,20) = sfc_fld%hice(i,j)
          grid_node(i,j,21) = sfc_fld%fice(i,j)
          grid_node(i,j,22) = sfc_fld%tisfc(i,j)
          grid_node(i,j,23) = sfc_fld%tprcp(i,j)
          grid_node(i,j,24) = sfc_fld%srflag(i,j)
          grid_node(i,j,25) = sfc_fld%snwdph(i,j)
          grid_node(i,j,26) = sfc_fld%shdmin(i,j)
          grid_node(i,j,27) = sfc_fld%shdmax(i,j)
          grid_node(i,j,28) = sfc_fld%slope(i,j)
          grid_node(i,j,29) = sfc_fld%snoalb(i,j)
          grid_node(i,j,30) = sfc_fld%oro(i,j)
          grid_node(i,j,31) = sfc_fld%sncovr(i,j)
          do l=1,lsoil
       ...
       do k=1,levs
        do j=1,lats_node_r
          ...
          do lon=1,lons_lat,NGPTC
            ...
            iblk=iblk+1
            do i=1,NJEFF
              grid_node(il,j,nrecs+3*lsoil+num_p2d+(l-1)*levs+k) = 
     &          phy_f3d(i,k,iblk,j,l)
              il=il+1
            enddo
          enddo
        enddo
