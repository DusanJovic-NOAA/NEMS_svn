!!!!!  ==========================================================  !!!!!
!!!!!              rrtm0 radiation package description             !!!!!
!!!!!  ==========================================================  !!!!!
!                                                                      !
!    the rrtm0 package includes these parts:                           !
!                                                                      !
!       'radlw_rrtm0_param.f'                                          !
!       'radlw_rrtm0_datatb.f'                                         !
!       'radlw_rrtm0_main.f'                                           !
!                                                                      !
!    the 'radlw_rrtm0_param.f' contains:                               !
!                                                                      !
!       'module_radlw_cntr_para'   -- control parameters set up        !
!       'module_radlw_parameters'  -- band parameters set up           !
!                                                                      !
!    the 'radlw_rrtm0_datatb.f' contains:                              !
!                                                                      !
...
!                                                                      !
!    the 'radlw_rrtm0_main.f' contains:                                !
!                                                                      !
...
!                                                                      !
!                                                                      !
!       'lwrad'     -- main rrtm0 lw radiation routine                 !
!          inputs:                                                     !
...
!                                                                      !
!       nov 1999,  ken campana  -- received originalcode from aer      !
...
!  ...  atomic weights for conversion from mass to volume mixing ratios
      real (kind=kind_phys) :: amd, amw, amdw, amdo3
...
!     aerosols(NPTS,NLAY,:) - aerosol optical properties               !
...
!! ---  optional outputs:
!!    real (kind=kind_phys), dimension(:,:), intent(out) :: hlw0
!!    type (proflw_type),    dimension(:,:), intent(out) :: flxprf
...
!  ---  set absorber amount
!test use
!           h2ovmr(k)= max(zero,qnm(iplon,k1)*amdw)                     ! input mass mixing ratio
!           h2ovmr(k)= max(zero,qnm(iplon,k1))                          ! input vol mixing ratio
!ncep model use
            h2ovmr(k)= max(zero,qnm(iplon,k1)*amdw/(1.0-qnm(iplon,k1))) ! input specific humidity
            o3vmr (k)= max(zero,o3mr(iplon,k1)*amdo3)                   ! input mass mixing ratio
!test use   o3vmr (k)= max(zero,o3mr(iplon,k1))                         ! input vol mixing ratio

            tem0 = (1.0 - h2ovmr(k))*amd + h2ovmr(k)*amw
...
!aer      if (iaersw == 1) then
!           do k = 1, NLAY
!             k1 = NLP1 - k
!             tauae(k) = aerosols(iplon,k1,1)
!           enddo
!         else
!           tauae(:) = zero
!aer      endif
          ...
          cldfrac(NLP1) = zero        ! padding value only
...
!  ---  set absorber amount
!test use
!           h2ovmr(k)= max(zero,qnm(iplon,k)*amdw)                     ! input mass mixing ratio
!           h2ovmr(k)= max(zero,qnm(iplon,k))                          ! input vol mixing ratio
!ncep use
            h2ovmr(k)= max(zero,qnm(iplon,k)*amdw/(1.0-qnm(iplon,k)))  ! input specific humidity
            o3vmr (k)= max(zero,o3mr(iplon,k)*amdo3)                   ! input mass mixing ratio
!test use   o3vmr (k)= max(zero,o3mr(iplon,k))                         ! input vol mixing ratio
...
!             colamt(k,6)=max( zero,     coldry(k)*gasvmr(iplon,k1,4))  ! o2 - not used
!             colamt(k,7)=max( zero,     coldry(k)*gasvmr(iplon,k1,5))  ! co - not used
            enddo
          else
            do k = 1, NLAY
              colamt(k,4) = zero     ! n2o
              colamt(k,5) = zero     ! ch4
!             colamt(k,6) = zero     ! o2 - not used
!             colamt(k,7) = zero     ! co - not used
            enddo
          endif
          ...
          if (icfclw == 1) then
            do k = 1, NLAY
              k1 = NLP1 - k
              wx(k,1) = max( zero, coldry(k)*gasvmr(iplon,k1,9) )   ! ccl4
              wx(k,2) = max( zero, coldry(k)*gasvmr(iplon,k1,6) )   ! cf11
              wx(k,3) = max( zero, coldry(k)*gasvmr(iplon,k1,7) )   ! cf12
              wx(k,4) = max( zero, coldry(k)*gasvmr(iplon,k1,8) )   ! cf22
            enddo
          else
            wx(:,:) = zero
          endif
...
!             colamt(k,6)=max( zero,     coldry(k)*gasvmr(iplon,k,4))  ! o2 - not used
!             colamt(k,7)=max( zero,     coldry(k)*gasvmr(iplon,k,5))  ! co - not used
            enddo
          else
            do k = 1, NLAY
              colamt(k,4) = zero     ! n2o
...
!             colamt(k,7) = zero     ! co - not used
          ...
          if (icfclw == 1) then
            do k = 1, NLAY
              wx(k,1) = max( zero, coldry(k)*gasvmr(iplon,k,9) )   ! ccl4
              wx(k,2) = max( zero, coldry(k)*gasvmr(iplon,k,6) )   ! cf11
              wx(k,3) = max( zero, coldry(k)*gasvmr(iplon,k,7) )   ! cf12
              wx(k,4) = max( zero, coldry(k)*gasvmr(iplon,k,8) )   ! cf22
            enddo
          else
            wx(:,:) = zero
          endif
...
!
      ...
      if (me == 0) then
        ...
        if (iaerlw == 1) then
          print *,'   This version of RRTM-LW does not include ',       &
     &            'aerosols effect.'
!not yet  print *,'   --- Include aerosols effect in LW'
        else
          print *,'   --- Aerosols effect is NOT included in LW'
        endif
...
!  ---  inputs:
      ...
      real (kind=kind_phys), dimension(0:NLP1), intent(in) :: cldfrac

      real (kind=kind_phys), dimension(:),  intent(in) :: cliqp, cicep, &
     &       reliq, reice, cdat1, cdat2, cdat3, cdat4

!  ---  outputs:
      real (kind=kind_phys), intent(out) :: taucloud(:,:)
...
!  ---  inputs:
      integer, intent(in)  ::  NLAY, NLP1, itr(NGPT,NLAY)

      real (kind=kind_phys), dimension(0:NLAY), intent(in) :: tz
      real (kind=kind_phys), dimension(0:NLP1), intent(in) :: cldfrac

      real (kind=kind_phys), dimension(NLAY), intent(in) :: tavel, delp

      real (kind=kind_phys), intent(in) :: taucloud(NBANDS,NLAY),       &
     &       semiss(NBANDS), pfrac(NGPT,NLAY)

!  ---  outputs:
      real (kind=kind_phys), dimension(0:NLAY), intent(out) ::          &
     &       htr, htrcl, totuflux, totdflux, totuclfl, totdclfl
...
!    totuflux(0:NLAY)     ! upward longwave flux (w/m2)                 !
!    totdflux(0:NLAY)     ! downward longwave flux (w/m2)               !
!    htr     (0:NLAY)     ! longwave heating rate (k/day)               !
!    totuclfl(0:NLAY)     ! clear sky upward longwave flux (w/m2)       !
!    totdclfl(0:NLAY)     ! clear sky downward longwave flux (w/m2)     !
...
!    efcfr1  (NBANDS,NLAY)! effective cloudy sky fraction               !
...
!  ---  outputs:
      real (kind=kind_phys), dimension(0:NLAY), intent(out) :: htr,     &
     &       htrcl, totuflux, totdflux, totuclfl, totdclfl
...
!  --- ... maximum/random cloud overlap
     ...
     &                        / (cldfrac(k-1) - cldfrac(k))
            ...
            if (facclr1u(k+1) > zero .or. facclr2u(k+1) > zero) then
              rat1 = 1.0
              rat2 = zero
            else
              rat1 = zero
     ...
     &                        / (cldfrac(k) - fmin)
            ...
            if (faccld1u(k+1) > zero .or. faccld2u(k+1) > zero) then
              rat1 = zero
              rat2 = 1.0
            else
              rat1 = zero
     ...
     &                        / (cldfrac(k+1) - cldfrac(k))
            ...
            if (facclr1d(k-1) > zero .or. facclr2d(k-1) > zero) then
              rat1 = 1.0
              rat2 = zero
            else
              rat1 = zero
     ...
     &                        / (cldfrac(k) - fmin)
            ...
            if (faccld1d(k-1) > zero .or. faccld2d(k-1) > zero) then
              rat1 = zero
              rat2 = 1.0
            else
              rat1 = zero
...
!  ---  inputs:
     ...
     &       colamt(NLAY,MAXGAS), wx(NLAY,MAXXSEC)

      integer, dimension(NLAY), intent(in) :: jp, jt, jt1, indself

!  ---  outputs:
      real (kind=kind_phys),dimension(NGPT,NLAY), intent(out) :: pfrac
      integer,              dimension(NGPT,NLAY), intent(out) :: itr
...
!
      ...
      do k = 1, NLAY
        do j = 1, NGPT
          ...
          itr(j,k) = max(0, min(N5000, itr(j,k)))
        enddo
      enddo
